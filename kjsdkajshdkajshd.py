...
class Image:
    ...
    def FindContours(self, upto, approxedges):
        """ Gaussian Blur to radius false detection """
        #img = self.ImgGrayBlurred
        img = cv2.GaussianBlur(self.ImgGray, (9, 9), 0)
        # Apply Circle hough transform with radius between 200,600 (because images size is 2000x2000 and
        # the dish size is about 50% of image)
        ...
        circles = circles.tolist()    # tabdile be lis

        for cir in circles:
            for x, y, r in cir:
                x, y, r = int(x), int(y), int(r)  #az 66 ta 70 dayeraro mikeshe doresham siyah mikone to ax asli
        ...
        cv2.waitKey(0)
        ret, img = cv2.threshold(img, 127, 255, cv2.THRESH_TOZERO)  #rangai ke tire tar az khakestari hastan meshki mikone  age kamrang tar bashe ( roshan tar ) kari bash nadare
        #cv2.imshow("",img )
        cv2.imshow("test", cv2.resize(img, (500, 500)))
        cv2.imshow("test2", cv2.resize(ret, (500, 500)))
        cv2.waitKey(0)

        _,cnts, hier = cv2.findContours(img, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)  #list be esme cnts dorost mikone tamame noghtehai ke dore dayere hasto moshakhat mikone
        cnts = sorted(cnts, key=cv2.contourArea, reverse=True)[:upto]   #sort mikone
        screenCnt = None
        for c in cnts:
            approx = cv2.approxPolyDP(c, 0.01 * cv2.arcLength(c, True), True)
            if len(approx) > approxedges:  #baraye inke tashkhis bede dayreh hast az in estefade mishe
                print 'circle'
                screenCnt = c
                break

        return screenCnt  #harkodom ke dayere beshe mirize to screenCnt

    def MaskImg(self, img, cnt, r, g, b, thick):  #1tike az ax ke laze negah midare baghiyasho hazv mikone
        stamp = False
        imgcopy = img.copy()
        ...
        return imgcopy, out
    #hame axsaro ke mikhaymo mask mikone doreshe ke nemikhaym meshki mikone dish moshakhas mishe

    def ExtractRoi(self, img, cnt):   # 4 line mikesh momas b adayere mibore az roye on khata
        x, y, width, height = cv2.boundingRect(cnt)
        roi = img[y:y + height, x:x + width]
        roigray = cv2.cvtColor(roi, cv2.COLOR_BGR2GRAY)
        return roi, roigray

    def IncreaseContrast(self, img, maxIntensity, alfa, beta, scaleFact):  # keyfiyate ax mibare bala
        imgcopy = (maxIntensity / alfa) * (img / (maxIntensity / beta)) ** scaleFact
        imgcopy = array(imgcopy, dtype=uint8)
        return imgcopy

    def ThresholdAndSmooth(self, img, maxIntensity, phi, theta, scaleFactor): #ax 1bar miyar sefya sefid mikone
...
############## here #######
    ...
    #cv2.imshow("images", cv2.resize(images[0], (500, 500)))
    #cv2.waitKey(0)
    circle = images[0].FindContours(10,
...
#######mask in all photo  with mask we crating up  ############
    for i in range(len(images_names)):
        masked_images[i], out_images[i] = images[0].MaskImg(images[i].ImgResized, circle, 0, 255, 0, 3)
        roi_images[i], roigray_images[i] = images[0].ExtractRoi(out_images[i], circle)   # cat the circle
        if i == 0:  #1th photo used
            diff_images[i] = roigray_images[i]
        else:   # har ax ba ax ghabli moghayese mikone tafavotesho mirizi to diff_image
            diff_images[i] = cv2.absdiff(roigray_images[i], roigray_images[i - 1])
            diff_images[i] = images[0].IncreaseContrast(diff_images[i], maxIntensity, phi, theta, scaleFactor) # taghire contrast image 0
    ...
    background = roigray_images[2]  # min must me 3 image
    for i in range(len(images_names)):
        backsub_images[i] = cv2.absdiff(roigray_images[i], background)  # tafavote ax mirzi inja
