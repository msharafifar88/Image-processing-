...
class Image:
    ...
    def ExtractRoi(self, image):
        ...
        circles = cv2.HoughCircles(img, cv2.HOUGH_GRADIENT, 2 , 20, param1=210, minRadius=350, maxRadius=520)
        print (circles)
        circles = np.uint16(np.around(circles))
        ...
        for w, h, r in circles[0]:
            ...
            y+=h
            rtot+=r
            cv2.circle(img, (w, h), r, (0, 0, 255), 3)
        # draw outer circle
        ...
        #cv2.imshow("test to show all circle", cv2.resize(img, (300, 300)))
        #cv2.waitKey(0)
