...
class Image:
    ...
    #     circles = cv2.HoughCircles(img, cv2.HOUGH_GRADIENT, 2, 20, param1=120 , minRadius=200, maxRadius=600)
    ...
    def ExtractRoi(self,image):
        img = cv2.GaussianBlur(self.ImgGray, (9, 9), 0)
        circles = cv2.HoughCircles(img, cv2.HOUGH_GRADIENT, 3, 20, param1=120 , minRadius=200, maxRadius=600)
        circles = np.uint16(np.around(circles))
        ...
        cropCoords = (max(0, y - cropSize[0] // 2), min(img.shape[0], y + cropSize[0] // 2),
                      max(0, x - cropSize[1] // 2), min(img.shape[1], x + cropSize[1] // 2))
        mask = np.zeros_like(image)
        mask = cv2.ellipse(mask, center=(x, y), axes=(int(cropCoords[0]*.69),int( cropCoords[2]*.63)), angle=0, startAngle=0, endAngle=360,
                           color=(255, 255, 255), thickness=-1)
        result = np.bitwise_and(image, mask)
        result_rgb = cv2.cvtColor(result, cv2.COLOR_BGR2RGB)
        cv2.imshow("tttt", cv2.resize(result_rgb, (300, 300)))
        cv2.waitKey(0)

        roi = result_rgb[int(cropCoords[0]*.65):int(cropCoords[1]*1.20), int(cropCoords[2]*.75):int(cropCoords[3]*1.15)]
