...
class Image:
    ...
    def ExtractRoi(self,image):
        ...
        circles = np.uint16(np.around(circles))    #tabdil mikone be tupel
        print circles
        ...
        cv2.imshow("imagetest",cv2.resize(img,(500 ,500)) )
        # cv2.waitKey(0)
        ...
        roi = image[int(cropCoords[0]*.65):int(cropCoords[1]*1.20), int(cropCoords[2]*.75):int(cropCoords[3]*1.15)]
        # roi = image[int(cropCoords[0]):int(cropCoords[1]), int(cropCoords[2]):int(cropCoords[3])]
        # roi = cv2.GaussianBlur(roi, (51, 51), 0)

        ret, roi = cv2.threshold(roi, 112, 255, cv2.THRESH_TOZERO)
